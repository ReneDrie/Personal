module.exports = function (grunt)
{
	grunt.registerTask('organize-imports', 'Organizes the imports of a TypeScript file', function(n)
	{
		...
		if(file.substring(dotIndex + 1) == "ts")
		{
			...
			{
				// leave all lowercase imports
				if (result[1].charAt(0) == result[1].charAt(0).toLowerCase())
				{
					imports += result[0] + "\n";
				}
				else
			...
			grunt.log.writeln(grunt.config('organize-imports.options.customCatalogFile'));
		...
		{
			grunt.log.error("No TypeScript file");
			return;
		}
	});
